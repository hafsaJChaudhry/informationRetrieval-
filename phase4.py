...
def main():
    ...
    collection = extract_data(file_list, word_freq_per_doc, doc_len, proc_times_cumulative) #extracts text, times and plot

    global_word_counter = collection[0]
    doc_freq_per_word = collection[1]
    term_freq_and_idf = collection[2]
    term_freq = collection[3]

    query = ["diet", "international", "affairs", "computer", "network", "hydrotherapy", "identity", "theft", "cats"]

    for word in query:
        print(word)
        print(doc_freq_per_word.get(word))
        print("********")
...
def extract_data(file_list, word_freq_per_doc, doc_len, proc_times_cumulative):
    ...
    return global_word_counter, doc_freq_per_word, term_freq_and_idf, term_freq
